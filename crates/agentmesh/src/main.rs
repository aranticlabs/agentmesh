...
use hooks::{
    install_detected_runtime_hooks, install_git_pre_commit_hook, install_runtime_hook,
    print_git_pre_commit_dry_run, print_runtime_install_dry_run, print_upgrade_dry_run,
    rewrite_installed_runtime_hooks, uninstall_runtime_hooks,
};
use inspect::{
    ReviewedDiffSummary, clear_reviewed_diff_state, doctor_json, inspect_repo, inspect_status_repo,
    integrity_exit_code, print_doctor, print_integrity, print_scan, print_status, print_versions,
    read_reviewed_diff_state, scan_json, snapshot_exit_code, status_json,
    write_reviewed_diff_state,
};
...
mod tests {
    ...
    use super::inspect::{
        HookOwnershipSnapshot, IntegritySnapshot, LockfileSnapshot, RepoSnapshot, RuntimeSnapshot,
        WatcherSnapshot, status_json,
    };
    use super::{
        CanonicalInstructionSource, Cli, Command, InitCommand, ParsedInitOptions,
        ParsedSyncOptions, SyncCommand, SyncTrigger, parsed_init_options, parsed_sync_options,
        sync_check_exit_code,
    };
    ...
    fn unknown_runtime_scan_reports_cursor_rules() {
        ...
        let unknown = match super::inspect::inspect_unknown_runtime_dirs(temp.path()) {
            Ok(unknown) => unknown,
